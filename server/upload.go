...
const (
	numUploadParts          = 64
	minUploadPartSize int64 = 95 * 1024 * 1024
	maxUploadPartSize int64 = 1024 * 1024 * 1024
)
	...
	for i := range b.Parts {
		...
		g.Go(func() error {
			for try := 0; try < maxRetries; try++ {
				...
				err := b.uploadChunk(ctx, http.MethodPatch, requestURL, r, part, opts)
				switch {
...
func (b *blobUpload) uploadChunk(ctx context.Context, method string, requestURL *url.URL, rs io.ReadSeeker, part *blobUploadPart, opts *RegistryOptions) error {
	...
	case resp.StatusCode == http.StatusTemporaryRedirect:
		...
		for try := 0; try < maxRetries; try++ {
			...
			if err := b.uploadChunk(ctx, http.MethodPut, redirectURL, rs, part, nil); err != nil {
